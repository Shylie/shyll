...
Linker::Linker(const std::map<std::string, std::string>& sources) : compiler(sources)
{
}
...
{
	bool success;
	std::map<std::string, std::map<std::string, Chunk>>* symbols = compiler.Compile(success);
	if (!success) { return BuildResult::CompilerError; }

	locs.clear();

	{
		std::vector<std::string> filesFoundIn;
		for (auto& [file, _symbols] : *symbols)
		{
			if ((*symbols)[file].find("!main") != (*symbols)[file].end())
			{
				chunk = (*symbols)[file]["!main"];
				filesFoundIn.push_back(file);
			}
		}
		...
		{
			std::cerr << "Multiple main functions definitions: ";
			for (size_t i = 0; i < filesFoundIn.size() - 1; i++)
			{
...
#pragma endregion
	...
	for (auto& [file, _symbols] : *symbols)
	{
		for (auto& [symbol, _chunk] : _symbols)
		{
